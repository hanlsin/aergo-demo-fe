<template>
  <div class="home">
    <div>
      <div v-show="!isLoading2" style="margin-left: 50px;margin-right: 50px;display: block">
        <li style="text-align: left" v-for="(value, key) in metadata">
          {{ key }}: {{ value }}
        </li>
    ...
    <div style="width: 100%; text-align: center">
      <router-link :to="{ name: 'newcontract', params: {} }"
        class="btn btn-primary rounded-pill"
        style="margin: 0">New Contract</router-link>
    </div>
    <div v-show="!isLoading" style="margin: 20px">
      <table style="width: 100%">
        ...
        <tr v-for="contract in contracts" :key="contract.block_height">
          ...
          <td><a href="#" v-on:click="goContract(contract.contract_id)"
            class="btn btn-primary rounded-pill"
            style="text-transform: none;margin: 0;padding-left: 5px;padding-right: 5px;margin-left: 5px;margin-right: 5px">
            {{ contract.contract_id.substr(0, 5) }}..{{ contract.contract_id.substr(-5) }}
          </a></td>
          <!--td>{{ contract.contract_id.substr(0, 5) }}...</td-->
          <td v-if="address == contract.issuer"><span style="color: #ff0097">USER</span></td>
          <td v-if="address != contract.issuer"><a href="#" v-on:click="goProfile(contract.issuer)"
            class="btn btn-outline-primary rounded-pill"
            style="text-transform: none;margin: 0;padding-left: 5px;padding-right: 5px;margin-left: 5px;margin-right: 5px">
            {{ contract.issuer.substr(0, 5) }}..{{ contract.issuer.substr(-5) }}
          </a></td>
          <!--td><router-link :to="{ name: 'contracts', params: { address: contract.issuer }}">
            {{ contract.issuer.substr(0, 5) }}...
          </router-link></td-->
          <td v-if="address == contract.receiver"><span style="color: #ff0097">USER</span></td>
          <td v-if="address != contract.receiver"><a href="#" v-on:click="goProfile(contract.receiver)"
            class="btn btn-outline-primary rounded-pill"
            style="text-transform: none;margin: 0;padding-left: 5px;padding-right: 5px;margin-left: 5px;margin-right: 5px">
            {{ contract.receiver.substr(0, 5) }}..{{ contract.receiver.substr(-5) }}
          </a></td>
...
export default {
  ...
  mounted: function() {
    ...
    this.$store.dispatch('getProfile', { useraddr:this.address }).then(data => {
    ...
    this.$store.dispatch('reqContractList', { address:this.address })
      .then(data => {
