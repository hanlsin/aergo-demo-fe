<template>
  <div class="login">
    ...
    <div v-show="!isLoading">
      <form ref="form">
        <input placeholder="Enter your encrypted key" v-model="encKey"
          class="input-box">
        <input placeholder="Enter your password" v-model="password"
          type="password" class="input-box">
        <router-link :to="{ name: 'signup', params: {} }" v-show="!isLoading"
          class="btn btn-outline-primary rounded-pill">Sign up</router-link>
        <a href="#" v-on:click="submit" v-show="!isLoading"
